...
if (time.getHours() >= 18 || time.getHours() <= 4) {
  document.querySelector(".container").style.background =
    "linear-gradient(204deg, rgba(0,0,0,1) 0%, rgba(0,35,64,1) 65%, rgba(0,79,56,1) 100%)";
} else {
  document.querySelector(".container").style.background =
    "linear-gradient(167deg, rgba(41,78,255,1) 0%, rgba(0,138,255,1) 38%, rgba(87,191,160,1) 100%)";
}
    ...
    fetch(final_url)
      ...
      .then((result) => {
        if (result.cod == "404") {
          ...
          if (
            result.weather[0].main.toLowerCase() ===
            result.weather[0].description.toLowerCase()
          ) {
